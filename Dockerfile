# node:22 (current LTS) ships an undici without the gzip/br decompression bug
# (`controller[kState].transformAlgorithm is not a function`) that crashed every
# Wikipedia fetch on node:20 and made all articles 404. Keep on an LTS.
