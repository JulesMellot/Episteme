...
interface WikiFromFilePageClientProps {
...
export function WikiFromFilePageClient({ initialLanguage }: WikiFromFilePageClientProps) {
  const router = useRouter();
  const [fileName, setFileName] = useState<string | null>(null);
  const [text, setText] = useState("");
  const [error, setError] = useState<string | null>(null);
  const [lang] = useState(() => {
    if (typeof window === "undefined") return initialLanguage || DEFAULT_WIKI_LANGUAGE;
  ...
  const uiCopy =
    uiLocale === "fr"
      ? {
          ...
          subtitle:
            "Uploade un fichier texte contenant un titre Wikipedia, ou une URL d'article, puis ouvre la page correspondante.",
          ...
          placeholder: "Colle ici un titre ou une URL Wikipedia...",
          parseError: "Impossible d'extraire un titre Wikipedia depuis ce contenu.",
          openArticle: "Ouvrir l'article",
          detectedTitle: "Titre detecte",
        }
      : {
